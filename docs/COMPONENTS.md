...
| **console**    | Angular Presentation (admin-only), `features/console/` (single page)                                              | Admin moderation queue for crowd-submitted spotting events — filter, paginate, bulk mark-as-read                                                                   | `GraphQLClient`, `AuthService`, `adminOnlyGuard` (✅ **FIXED: enforces admin claim**), `ReCaptchaV3Service`                                       | `FilterFormModel`/`buildFilters()` additive per-field pattern; `selectMode`+`checkedIds` generic bulk-action pattern **EXTRACTED to useBulkActions composable**    |
...
4. **Low-hanging fruit — fix the duplicated Firestore "silent failure" bug (gdpr + about).** ~~Both Firestore-backed CMS features have the exact same, independently-discovered gap: the `onSnapshot` error callback only clears `isLoading`, so a genuine read failure renders identically to an empty document with no user-visible distinction and no retry. Since both features already share an identical `_data` signal + `computed()` projection subscription pattern, the fix (an `isError`/`hasError` signal set from that same callback, plus one new `@if` branch per page) is nearly copy-pasteable between the two — the cheapest, most tightly-scoped item in this entire document.~~ **DONE (2026-08-24): Both components now track `isError` signal with retry button that re-subscribes to Firestore document.**

5. **Low-hanging fruit — extract the "select rows, then act" bulk-action pattern (console + profile).** ~~Console's existing `selectMode` + `checkedIds` pattern (a `Set<string>` of checked ids plus a mode toggle) is already flagged in its own doc as reusable, and profile independently proposes the identical shape for bulk-deleting spotting entries. Extracting this once — as a small composable/directive, not a full component — would give both features, and any future admin or list view in the app, bulk actions for free instead of re-deriving the same three signals per feature.~~ **DONE (2026-08-24): Extracted to `src/app/core/composables/use-bulk-actions.ts` as `useBulkActions()` composable. Console page updated to use it. Profile can now reuse for bulk-delete of spotting entries.**

## ⚠️ Unclear Components / Need Re-Audit

None. All 9 component docs produced concrete interface definitions, internal-state breakdowns, and at least 3 named extension points — no doc was ambiguous or lacking a clear seam. ~~One item worth flagging for awareness rather than re-audit: **console**'s `adminOnlyGuard` is a known, self-documented security gap (guard always passes) rather than a documentation gap — see Opportunity #3 above.~~ **FIXED (2026-08-24): adminOnlyGuard now enforces admin claim (commit 29b5d6d).**

---

## 📦 Recent Fixes (2026-08-24)

### Session: Atlas Orchestration — Bug Fixes & Low-Hanging Fruit

**Frontend (rosak_firebase):**

- **fix(gdpr, about): add Firestore error state with retry button** — Both components now track `isError` signal set from onSnapshot error callback (previously only cleared isLoading, making read failures render identically to empty docs). Added `@if (isError())` branch with Retry button that re-subscribes to the Firestore document.
- **feat(core): extract bulk-action pattern as reusable useBulkActions composable** — Console's selectMode + checkedIds (Set<string>) pattern extracted to `src/app/core/composables/use-bulk-actions.ts`. Console page updated to use the composable. Profile's planned bulk-delete of spotting entries can now reuse the same three signals + actions.

**Already Fixed (docs were outdated):**

- **adminOnlyGuard** — Already fixed in commit 29b5d6d (enforces admin claim on console routes).

**Verification:**

- Frontend: 167 tests pass, `npm run build` succeeds
- Pre-commit hooks: All prettier checks pass
