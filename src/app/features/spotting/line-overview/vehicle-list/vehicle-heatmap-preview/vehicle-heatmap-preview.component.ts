...
@Component({
  ...
  template: `
    ...
    <div
      class="bg-popover text-popover-foreground border-border max-h-96 w-[960px] overflow-y-auto rounded-lg border p-3 shadow-md"
    >
...
export class VehicleHeatmapPreviewComponent {
  ...
  protected readonly _trends = computed(() =>
    toSpottingActivityPoints(this.resource.data()?.vehicles[0]?.spottings ?? []),
  );
}
