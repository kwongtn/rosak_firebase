...
import {
  LINE_SPOTTING_GRID_QUERY,
  LineSpottingGridQueryData,
  ...
  template: `
    <div class="flex flex-col gap-3">
      @if (isAwaitingCurrentView()) {
        ...
        <app-retry-banner
          [resource]="resource"
          message="Couldn't load this period's spotting data."
        />
      } @else {
...
export class VehicleSpottingGridComponent {
  ...
  protected readonly hoveredCell = signal<{
    vehicleId: string;
    label: string;
    dateKey: string;
  } | null>(null);
  ...
  protected readonly _currentSection = computed(
    () => this.sections().find((s) => s.typeId === this.currentSectionId()) ?? null,
  );
  ...
  protected readonly isAwaitingCurrentView = computed(
    () => !this.resource.data() && !this.resource.hasError(),
  );
    ...
    afterNextRender(() => {
      ...
      const bodyScrollWidthRef = effect(
        () => {
          ...
          bodyScrollResizeObserver = new ResizeObserver(() =>
            this.bodyScrollWidth.set(el.clientWidth),
          );
      ...
      const onPageScroll = () => {
        ...
        const past = containerEl
          ? containerEl.getBoundingClientRect().bottom <= this.stickyOffset()
          : false;
        ...
        this.currentSectionId.set(
          currentIdx >= 0 ? (anchors[currentIdx].nativeElement.dataset["typeId"] ?? null) : null,
        );
