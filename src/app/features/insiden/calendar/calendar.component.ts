      ...
      <div class="flex items-center justify-between">
        <h2 class="flex items-baseline gap-1">
          ...
          @if (jumpCounting()) {
            <svg
              viewBox="0 0 24 24"
              class="text-primary size-3.5 shrink-0 -rotate-90"
              aria-hidden="true"
            >
              <circle
                cx="12"
...
export class IncidentCalendarComponent implements OnDestroy {
   ...
   * written to at all). */
  protected readonly viewedMonth = linkedSignal(() =>
    startOfMonth(dateFromKey(this.selectedDate())),
  );
  ...
  protected readonly displayedMonth = computed(
    () => this.pendingMonth() ?? this.viewedMonth().getUTCMonth(),
  );
  protected readonly displayedYear = computed(
    () => this.pendingYear() ?? this.viewedMonth().getUTCFullYear(),
  );
  ...
  /** Long-term and open-ended (null endDatetime) incidents only show a dot on their first
   * day — they already have their own permanent home in the "Ongoing & Long-Running" section
   * regardless of the selected day, so counting them again on every day they span would just
   * repeat the same dot across an entire month (sometimes several) without telling the viewer
   * anything new. The first day still gets a dot so the viewer can discover the incident from
   * the calendar grid. */
  private readonly _countsByDate = computed(() => {
    ...
    for (let i = 0; i < 42; i++) {
      ...
      for (const incident of this.incidents()) {
        ...
        if (
          (incident.longTerm || incident.endDatetime === null) &&
          incident.startDatetime.slice(0, 10) !== key
        ) {
          continue;
        }
