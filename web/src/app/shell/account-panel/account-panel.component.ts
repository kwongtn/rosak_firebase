...
import { REQUEST_VERIFICATION_CODE_MUTATION, RequestVerificationCodeData } from "./data/account-panel.queries";

const COUNTDOWN_SECONDS = 60;
const LOGOUT_CONFIRM_WINDOW_MS = 3000;
const TELEGRAM_LINKING_WIKI_URL = "https://github.com/kwongtn/rosak_firebase/wiki/Linking-to-Telegram";
    ...
    template: `
        <div class="flex h-full flex-col gap-4">
            <div class="flex items-center gap-3">
                <span class="bg-muted text-muted-foreground relative flex size-14 shrink-0 items-center justify-center overflow-hidden rounded-full">
                    @if (photoUrl() && !avatarErrored()) {
                        <img [src]="photoUrl()" alt="" class="size-full object-cover" (error)="avatarErrored.set(true)" />
                    } @else {
            ...
            <button hlmBtn [variant]="logoutArmed() ? 'destructive' : 'outline'" class="mt-auto" (click)="onLogoutClick()">
...
export class AccountPanelComponent implements OnDestroy {
    ...
    protected async requestCode(): Promise<void> {
        ...
        try {
            ...
            this.toast.error("Couldn't request a code", err instanceof Error ? err.message : "Unknown error");
        } finally {
    ...
    protected onLogoutClick(): void {
        if (!this.logoutArmed()) {
            ...
            this.logoutArmedTimeout = setTimeout(() => this.logoutArmed.set(false), LOGOUT_CONFIRM_WINDOW_MS);
            return;
        }
