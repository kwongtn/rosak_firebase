...
@Component({
    ...
    template: `
        ...
        @if (_isOpen() && _filtered().length > 0) {
            <ul class="bg-popover text-popover-foreground border-border absolute z-50 mt-1 max-h-64 w-max min-w-full overflow-x-clip overflow-y-auto rounded-lg border py-1 shadow-md">
            ...
            <div class="bg-popover text-muted-foreground border-border absolute z-50 mt-1 w-full rounded-lg border px-2.5 py-1.5 text-sm shadow-md">
                {{ emptyMessage() }}
            </div>
        }
    `,
...
export class HlmCombobox<T> {
    ...
    readonly filterFn = model<((items: ComboboxItem<T>[], query: string) => ComboboxItem<T>[]) | undefined>(
        undefined
    );
