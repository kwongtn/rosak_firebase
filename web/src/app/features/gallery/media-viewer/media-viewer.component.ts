...
import { Component, HostListener, OnDestroy, PLATFORM_ID, inject, input, output } from "@angular/core";
...
@Component({
    ...
    template: `
        ...
        <div class="flex min-h-0 flex-1 flex-col overflow-y-auto lg:flex-row lg:overflow-hidden" (click)="$event.stopPropagation()">
            <div class="flex flex-1 items-center justify-center p-3 lg:min-h-0">
                <img [src]="media().file?.url ?? '/image-not-found.png'" alt="" class="max-h-[70vh] max-w-full rounded-lg object-contain lg:max-h-full" />
            </div>
