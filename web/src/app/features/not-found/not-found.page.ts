...
@Component({
    ...
    imports: [RouterLink, HlmButton, HlmSkeleton, LineStatusBadge, AppNavComponent, AppFooterComponent],
    template: `
        <app-nav />
        <div class="mx-auto flex min-h-screen w-full flex-col p-4 sm:p-6 lg:w-[90%]">
            <main class="flex flex-1 flex-col items-center justify-center gap-8 py-16 text-center">
                <div class="flex flex-col items-center gap-4">
                    <span class="text-muted-foreground text-xs font-semibold tracking-widest uppercase">{{ message().eyebrow }}</span>
                    <div class="flex items-center gap-4">
                        <svg viewBox="0 0 24 24" class="text-muted-foreground/40 size-14 shrink-0" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
                ...
                @if (attemptedPath()) {
                    <code class="bg-muted text-muted-foreground max-w-full truncate rounded-md px-3 py-1.5 text-xs">
                        {{ attemptedPath() }}
                    </code>
                }
                ...
                @if (petPic(); as pet) {
                    <div class="flex flex-col items-center gap-2 pt-4">
                        ...
                        <img [src]="pet.url" [alt]="'A random ' + pet.kind" class="max-h-72 rounded-lg object-cover shadow-md" />
                    </div>
