...
import { CalendarIncident, CalendarIncidentSeverity, ChronologyIndicator } from "../data/insiden.queries";
...
function defaultChronology(incident: CalendarIncident): CalendarIncident["chronologies"] {
    const entries: CalendarIncident["chronologies"] = [
        { order: 0, indicator: "BLUE", datetime: incident.startDatetime, content: "Start of incident", sourceUrl: null },
    ];
    if (incident.endDatetime) {
        entries.push({ order: 1, indicator: "GREEN", datetime: incident.endDatetime, content: "Issue resolved", sourceUrl: null });
    }
    return entries;
}
...
@Component({
    ...
    imports: [DatePipe, MarkdownComponent, HlmButton, HlmBadge, ...HlmCardImports, PhotoPickerComponent],
...
export class IncidentCardComponent implements OnDestroy {
    ...
    protected readonly isPhotosCompressing = computed(() => this.photoPickerRef()?.isCompressing() ?? false);
    ...
    protected readonly chronology = computed(() => {
        const incident = this.incident();
        const entries = incident.chronologies.length > 0 ? incident.chronologies : defaultChronology(incident);
    ...
    protected uploadPhotos(): void {
        ...
        this.toast.info("Photo upload queued", "Please wait for uploads to complete before closing this tab.");
