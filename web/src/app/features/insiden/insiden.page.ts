...
export class InsidenPage {
    ...
    protected readonly allIncidents = computed(() => this.incidentsResource.data()?.calendarIncidents ?? []);
    ...
    protected readonly pinned = computed(() => {
        const date = this.selectedDate();
        return this._sorted().filter((incident) => incident.endDatetime === null && !incidentCoversDate(incident, date));
    });
