    ...
    template: `
        <div class="flex flex-col gap-3">
            ...
            >
                ...
                <p class="text-muted-foreground text-xs">Lifts &amp; escalators — status tracking coming soon</p>
            </div>
                                ...
                                <div class="flex flex-col gap-1.5">
                                    @for (asset of station.assets; track asset.id) {
                                        <div class="flex items-center gap-2 text-xs">
                                            <span hlmBadge variant="outline">{{ ASSET_TYPE_LABEL[asset.assetType] }}</span>
...
export class StationAssetsSectionComponent {
    ...
    protected copyAnchorLink(): void {
        ...
        navigator.clipboard
            .writeText(url)
            .then(() => this.toast.success("Link copied", "Direct link to Station Assets copied to clipboard."))
            .catch(() => this.toast.error("Couldn't copy link"));
    }

    protected readonly resource = graphqlResource<LineStationAssetsQueryData, LineStationAssetsQueryVars>(() => ({
