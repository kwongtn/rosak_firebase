...
const TOOLTIP_DATE_LABEL = new Intl.DateTimeFormat("en-US", { month: "long", day: "numeric", year: "numeric", timeZone: "UTC" });
...
@Component({
    ...
    template: `
        <details class="rounded-lg border">
            <summary class="text-muted-foreground hover:text-foreground cursor-pointer px-3 py-2 text-sm font-medium select-none">
            ...
            <div class="border-t p-3">
                ...
                @if (resource.isLoading()) {
                    ...
                    <p class="text-muted-foreground text-sm">Couldn't load fleet status history for this line.</p>
                } @else if (columns().length === 0) {
                    <p class="text-muted-foreground text-sm">No fleet status history recorded yet for this source.</p>
                } @else {
                    ...
                    <div class="bg-muted/50 mb-2 flex min-h-9 flex-wrap items-center gap-x-3 gap-y-1 rounded-md px-2.5 py-1.5 text-xs">
                        @if (hoveredCol(); as col) {
                            ...
                            @for (seg of col.segments; track seg.status) {
                                <span class="flex items-center gap-1">
                                    ...
                                    {{ statusLabel(seg.status) }} — {{ seg.count }} ({{ seg.percent | number: "1.0-0" }}%)
                                </span>
                            }
                        } @else {
                    ...
                    <div class="flex gap-2">
                        ...
                        <div class="text-muted-foreground flex h-32 w-9 flex-col justify-between pb-px text-right text-[10px] tabular-nums">
                            <span>100%</span>
                            <span>75%</span>
                            <span>50%</span>
                            <span>25%</span>
                        ...
                        <div class="min-w-0 flex-1 overflow-x-auto">
                            <div #chartRow class="relative flex h-32 items-end gap-px" (mouseleave)="hoveredCol.set(null)">
                            ...
                            <div class="relative mt-1 h-4">
                                @for (tick of xTicks(); track tick.dateKey) {
                                    <span class="text-muted-foreground absolute text-[10px]" [style.left.px]="tick.offsetPx">
                                        {{ tick.label }}
                                    </span>
                                }
                            </div>
...
export class VehicleStatusTrendComponent {
    ...
    protected readonly activeDisclaimer = computed(() => this.dataSources.find((s) => s.id === this.selectedSource())?.disclaimer ?? null);
        ...
        return [...byDate.entries()]
            ...
            .map(([dateKey, perStatus]) => {
                ...
                const segments: DaySegment[] = (Object.entries(perStatus) as [VehicleStatus, number][])
                    ...
                    .map(([status, count]) => ({ status, count, percent: total === 0 ? 0 : (count / total) * 100 }));
    ...
    protected readonly xTicks = computed<XTick[]>(() => {
        ...
        cols.forEach((col, i) => {
            ...
            if (month !== lastMonth) {
                ticks.push({ dateKey: col.dateKey, label: MONTH_TICK_LABEL.format(new Date(`${col.dateKey}T00:00:00Z`)), offsetPx: i * BAR_STEP_PX });
                lastMonth = month;
            }
        });
        return ticks;
