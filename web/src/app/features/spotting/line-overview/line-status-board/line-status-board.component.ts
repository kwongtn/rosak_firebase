...
export class LineStatusBoardComponent {
    ...
    protected readonly _mostRecent = computed<VehicleRow | undefined>(() =>
        [...this._vehicles()].filter((v) => v.lastSpottingDate).sort((a, b) => (a.lastSpottingDate! > b.lastSpottingDate! ? -1 : 1))[0]
    );
    ...
    protected readonly _stats = computed<BoardStat[]>(() => {
        ...
        return [
            ...
            { label: "Avg Spottings / Vehicle", value: total === 0 ? "—" : (totalSpottings / total).toFixed(1) },
            {
                label: "Never Spotted",
