...
export class ReportFormComponent {
    ...
    protected readonly vehicleItems = computed<ComboboxItem<string>[]>(() =>
        this._vehiclesForLine().map((v) => ({ label: v.identificationNo, value: v.id, meta: { status: v.status } }))
    );
    ...
    protected readonly vehicleFilterFn = computed(() => {
        ...
        return (items: ComboboxItem<string>[], query: string) => {
            ...
            const heuristicMatches = new Set(numberSeenToSetNumbers(query, lineId).map((s) => s.toLowerCase()));
            return items.filter(
                (item) =>
    ...
    protected readonly isAbnormalVehicle = computed(() => ABNORMAL_STATUSES.has(this.model().selectedVehicleStatus));
    ...
    private readonly stationLinesResource = graphqlResource<StationLinesQueryData, StationLinesQueryVars>(() => {
    ...
    protected readonly stationOptions = computed(() => this.stationLinesResource.data()?.stationLines ?? []);
    ...
    async submit(): Promise<void> {
        ...
        if (this.model().type === "LOCATION" && !this.capturedLocation()) {
            this.toast.error("Location not captured", "Please wait for location access, or pick a different type.");
            return;
        }
        if (this.isPhotosCompressing()) {
            ...
            this.toast.error("Photos still processing", "Please wait a moment for photo compression to finish.");
            return;
        }
        ...
        try {
                ...
                const data = await this.graphql.request<AddSpottingEventData, AddSpottingEventVars>(
                    ADD_SPOTTING_EVENT_MUTATION,
                    {
                        data: {
                            ...
                            originStation: m.type === "BETWEEN_STATIONS" ? m.originStation : m.type === "AT_STATION" ? m.atStation : null,
            ...
            if (ok) {
                this.toast.success(
                    ...
                    this.photos().length > 0 ? "Please wait for uploads to complete before closing this tab." : undefined
                );
                this.clear();
                this.submitted.emit();
            }
