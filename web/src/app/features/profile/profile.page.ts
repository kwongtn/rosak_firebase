...
import { GET_USER_DATA_QUERY, GetUserDataData, GetUserDataVars, UserData } from "./data/profile.queries";
            ...
            <main class="flex flex-1 flex-col gap-6">
                @if (!isAuthReady()) {
                    ...
                    @if (_isLoading()) {
                        ...
                        <spotting-activity-heatmap [data]="user.spottingTrends" [totalAllTime]="user.spottingsCount" />
                        <app-my-spottings />
                    } @else {
                        <p class="text-destructive text-sm">Couldn't load your profile. Please try again shortly.</p>
                    }
                } @else {
                    <div class="border-border flex flex-col items-center gap-2 rounded-lg border p-8 text-center">
                        ...
                        @if (!auth.isLoggedIn()) {
                            <button hlmBtn size="sm" class="mt-2" (click)="auth.login()">Log in to view your profile</button>
                        }
                    </div>
                }
            </main>
