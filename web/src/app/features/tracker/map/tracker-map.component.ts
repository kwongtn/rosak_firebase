...
export class TrackerMapComponent implements OnDestroy {
    ...
    private init(): void {
        ...
        this.scene.addImage("marker", "https://gw.alipayobjects.com/mdn/antv_site/afts/img/A*BJ6cTpDcuLcAAAAAAAAAAABkARQnAQ");
        this.scene.addImage("arrow", "https://gw.alipayobjects.com/zos/bmw-prod/ce83fc30-701f-415b-9750-4b146f4b3dd6.svg");
                    ...
                    if (!this.seenRtKeys.has(key) && this.scene) {
                        ...
                        queueMicrotask(() => {
                            if (this.seenRtKeys.has(key)) {
                                this.rtControllers.set(key, new RtMarkerLayerController(scene, source, this.injector, iconModeForSourceKey(key)));
                            }
                        });
                    }
                }
    ...
    private addStopsLayer(key: string, source: StaticSource): void {
        ...
        const layer = new PointLayer({}).source(source.stops()).shape("circle").size(4).color("#f59e0b").style({ opacity: 0.8 });
