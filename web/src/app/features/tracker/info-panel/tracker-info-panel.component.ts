...
import { Component, ElementRef, PLATFORM_ID, computed, effect, inject, input, output, signal, untracked, viewChild } from "@angular/core";
...
function highlightJson(json: string): string {
    const pattern = /("(\\u[a-fA-F0-9]{4}|\\.|[^"\\])*"(\s*:)?|\btrue\b|\bfalse\b|\bnull\b|-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)/g;
    return escapeHtml(json).replace(pattern, (match) => {
        let cls = "text-sky-600 dark:text-sky-400"; // number
        if (match.startsWith("\"")) {
            cls = match.endsWith(":") ? "text-violet-600 dark:text-violet-400" : "text-emerald-600 dark:text-emerald-400"; // key : string
...
@Component({
    ...
    template: `
        <hlm-sheet [open]="open()" (openChange)="openChange.emit($event)" [side]="fullscreen() ? 'full' : 'bottom'">
            ...
            <div hlmSheetBody class="overflow-x-auto pt-0!">
                ...
                <div #tabsRow class="bg-popover sticky top-0 z-20 mb-4 flex flex-wrap items-center gap-1.5 pt-5 pb-2">
                    <button hlmBtn [variant]="view() === 'overview' ? 'default' : 'outline'" size="sm" (click)="view.set('overview')">
                        Overview
                    </button>
                    @if (rows().length > 0) {
                        <button hlmBtn [variant]="view() === 'table' ? 'default' : 'outline'" size="sm" (click)="view.set('table')">Table</button>
                    }
                    <button hlmBtn [variant]="view() === 'json' ? 'default' : 'outline'" size="sm" (click)="view.set('json')">Raw JSON</button>
                    ...
                    @if (rtSource(); as source) {
                        ...
                        <button
                            ...
                            [attr.aria-label]="isStale() ? 'Newer data available — click to update' : 'Time until next refresh'"
                            [title]="isStale() ? 'Newer data available — click to update' : 'Time until next refresh'"
                            (click)="refreshSnapshot()"
                        >
                            @if (isStale()) {
                                <span class="text-destructive flex size-4 items-center justify-center font-bold">!</span>
                                <span class="text-muted-foreground">New data — click to update</span>
                            } @else {
                                <svg width="16" height="16" viewBox="0 0 18 18" class="-rotate-90" aria-hidden="true">
                                    <circle cx="9" cy="9" r="8" fill="none" stroke="currentColor" stroke-width="2" class="text-muted-foreground/20" />
                                    <circle
                                        cx="9"
                                        cy="9"
                                        r="8"
                ...
                @if (view() === "overview") {
                    <div class="flex flex-col gap-4 text-sm">
                        ...
                        @if (kind() === "railway") {
                            <p class="text-muted-foreground">
                                This layer is a single always-on/off overlay rather than a set of individually tracked items, so there's no
                                per-row table or breakdown to show here — just whether it's currently applied to the map.
                            </p>
                        }
                    </div>
                } @else if (view() === "table") {
                                    ...
                                    <th class="p-0 font-medium whitespace-nowrap">
                                        ...
                                        >
                                            ...
                                            @if (sortState()?.col === col) {
                                                <span class="text-muted-foreground">{{ sortState()?.dir === 1 ? "▲" : "▼" }}</span>
                                            }
                                        </button>
                                    </th>
                                }
                    ...
                    <button hlmBtn variant="outline" size="sm" class="mt-3" (click)="onExportCsv()">Export CSV</button>
                } @else {
                    <div class="flex flex-col gap-2">
                        <button hlmBtn variant="outline" size="sm" class="self-start" (click)="onCopyJson()">
                            {{ copied() ? "Copied!" : "Copy JSON" }}
                        </button>
                        <pre class="bg-muted max-h-[50vh] overflow-auto rounded-lg p-3 text-xs" [innerHTML]="highlightedJson()"></pre>
                    </div>
                }
            </div>
        </hlm-sheet>
...
export class TrackerInfoPanelComponent {
    ...
    protected readonly ringDashOffset = computed(() => {
        ...
        return RING_CIRCUMFERENCE * (1 - (source ? Math.max(0, Math.min(100, source.percentRemaining())) : 0) / 100);
    });
    ...
    protected readonly isStale = computed(() => this.kind() === "realtime" && this._liveVehicles() !== this.snapshotVehicles());

    private readonly _stops = computed(() => {
        const key = this.sourceKey();
        if (this.kind() !== "stops" || !key) {
            return [] as Array<{ id?: string | number | undefined; properties: Record<string, unknown> | null; geometry: { coordinates: number[] } }>;
        }
    ...
    protected readonly tableColumns = computed<string[]>(() => (this.rows().length > 0 ? Object.keys(this.rows()[0]) : []));
