                        ...
                        @if (!expanded()) {
                            ...
                            <span class="flex min-w-0 flex-wrap items-center gap-1 overflow-hidden">
                                @if (layerSelection.realtimeCount() > 0) {
                                    <span class="bg-muted text-muted-foreground rounded-full px-1.5 py-0.5 text-xs tabular-nums">RT: {{ layerSelection.realtimeCount() }}</span>
                                }
                                @if (layerSelection.railwayCount() > 0) {
                                    <span class="bg-muted text-muted-foreground rounded-full px-1.5 py-0.5 text-xs tabular-nums">Route: {{ layerSelection.railwayCount() }}</span>
                                }
                                @if (layerSelection.stopsCount() > 0) {
                                    <span class="bg-muted text-muted-foreground rounded-full px-1.5 py-0.5 text-xs tabular-nums">Stops: {{ layerSelection.stopsCount() }}</span>
                                }
                            </span>
                        } @else if (layerSelection.selectedCount() > 0) {
                            <span class="bg-primary text-primary-foreground inline-flex size-5 shrink-0 items-center justify-center rounded-full text-xs tabular-nums">
