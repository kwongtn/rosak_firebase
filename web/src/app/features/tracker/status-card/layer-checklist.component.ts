...
import { REALTIME_LAYER_CHECKBOXES, STOPS_LAYER_CHECKBOXES, RAILWAY_LINE_SOURCE, LayerCheckbox } from "../data/layer-config";
...
import { InfoPanelKind, TrackerInfoPanelComponent } from "../info-panel/tracker-info-panel.component";
...
const SUMMARY_CLASS = "bg-card sticky top-0 z-10 flex cursor-pointer items-center justify-between gap-2 py-1 text-sm font-medium";
            ...
            <details open class="group">
                <summary [class]="SUMMARY_CLASS">
                    <span class="flex items-center gap-1.5">
                        <svg viewBox="0 0 24 24" class="text-muted-foreground size-3.5 shrink-0 transition-transform duration-150 group-open:rotate-180" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                            <path d="m6 9 6 6 6-6" />
                        </svg>
                        Realtime layer
                        @if (layerSelection.realtimeCount() > 0) {
                            <span class="bg-primary text-primary-foreground inline-flex h-5 min-w-5 items-center justify-center rounded-full px-1 text-xs font-normal tabular-nums">
                    ...
                    >
                        ...
                        @for (option of refreshIntervalOptions; track option.value) {
                            <option [value]="option.value" [selected]="option.value === refreshIntervalOptionValue()">
                                {{ option.label }}
                            </option>
                        }
                    </select>
                                ...
                                <span class="inline-flex size-7 items-center justify-center">
                                    @if (gtfsRealtime.sources()[checkbox.value]; as source) {
                                        @if (!source.hasLoadedOnce()) {
                                            ...
                                            <svg class="text-muted-foreground size-3.5 [animation-direction:reverse]" style="animation: spin 1s linear infinite" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                                            ...
                                            <button
                                                ...
                                                (click)="openErrorKey.set(openErrorKey() === checkbox.value ? null : checkbox.value)"
                                            >
                            ...
                            @if (openErrorKey() === checkbox.value) {
                                ...
                                >
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <button hlmBtn size="sm" variant="ghost" (click)="openErrorKey.set(null)">Close</button>
                                    </div>
                                </div>
                            }
                        </li>
            ...
            <details class="group">
                ...
                <ul class="mt-2 flex flex-col gap-2">
                    <li class="flex items-center justify-between gap-2 text-sm">
                        <label class="flex min-w-0 items-center gap-2">
                            <hlm-checkbox [checked]="layerSelection.railwayChecked()" (checkedChange)="layerSelection.toggleRailway()" />
...
export class LayerChecklistComponent {
    ...
    protected openPanel(kind: InfoPanelKind, key: string | null, label: string, docsUrl: string): void {
    ...
    protected readonly refreshIntervalOptionValue = computed(() => refreshIntervalToOptionValue(this.gtfsRealtime.refreshIntervalMs()));
