...
@Component({
    ...
    template: `
        ...
        >
            <svg [attr.width]="SVG_SIZE" [attr.height]="SVG_SIZE" viewBox="0 0 22 22" class="-rotate-90" aria-hidden="true">
                <circle cx="11" cy="11" r="9" fill="none" stroke="currentColor" stroke-width="2.5" class="text-muted-foreground/20" />
                <circle
                    cx="11"
                    cy="11"
                    r="9"
...
export class CountdownRingComponent {
    ...
    protected readonly dashOffset = computed(() => CIRCUMFERENCE * (1 - Math.max(0, Math.min(100, this.source().percentRemaining())) / 100));
