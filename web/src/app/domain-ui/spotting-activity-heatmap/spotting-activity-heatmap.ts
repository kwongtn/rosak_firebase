...
@Component({
    ...
    template: `
        <div #container class="flex flex-col gap-3">
            <div class="flex flex-col gap-3" [class.flex-row]="useClassicLayout()" [class.items-start]="useClassicLayout()">
                @if (!hideYearPicker()) {
                <div class="flex flex-wrap gap-1.5" [class.flex-col]="useClassicLayout()" [class.order-last]="useClassicLayout()">
                ...
                <div class="min-w-0 flex-1">
                    @if (useClassicLayout()) {
                        ...
                        <div class="grid gap-1" style="grid-template-columns: repeat(auto-fill, minmax(18px, 1fr));">
        ...
        <ng-template #cellTpl let-day>
            <div class="relative" (mouseenter)="onHoverStart(day, $event)" (mouseleave)="hoveredDate.set(null)">
                <div
                    tabindex="0"
                ...
                @if (day.inYear && day.date === todayKey) {
                    ...
                    <span class="ring-foreground/80 pointer-events-none absolute inset-0 animate-pulse rounded-sm ring-2" aria-hidden="true"></span>
                }
                @if (day.inYear && hoveredDate() === day.date) {
                    <app-spotting-count-tooltip [dateKey]="day.date" [count]="day.count" [breakdown]="day.breakdown" [above]="tooltipAbove()" />
                }
            </div>
        </ng-template>
    `,
...
export class SpottingActivityHeatmap {
    ...
    private readonly _countsByDate = computed(() => {
        ...
        for (const [dateKey, points] of this._pointsByDate()) {
            map.set(dateKey, points.reduce((sum, p) => sum + p.count, 0));
        }
        return map;
    });
    ...
    protected readonly weeks = computed<WeekColumn[]>(() => {
        ...
        while (cursor <= gridEnd) {
            ...
            for (let i = 0; i < 7; i++) {
                ...
                days.push({ date: key, count: inYear ? counts.get(key) ?? 0 : 0, inYear, breakdown: inYear ? breakdown : [] });
     ...
     * calendar order. */
    protected readonly flatDays = computed(() => this.weeks().flatMap((week) => week.days.filter((day) => day.inYear)));
    ...
    protected readonly gridTemplateColumns = computed(() => `repeat(${this.weeks().length}, minmax(12px, 1fr))`);
    ...
    protected readonly useClassicLayout = computed(() => this.containerWidth() >= this.classicLayoutMinWidth());
    ...
    constructor() {
        afterNextRender(() => {
            ...
            requestAnimationFrame(() => requestAnimationFrame(() => this.containerWidth.set(el.getBoundingClientRect().width)));
        });
    }
