...
if (!hasAuthToken()) {
  console.log(
    "[sentry-sourcemaps] No Sentry auth token found (.sentryclirc / SENTRY_AUTH_TOKEN) — skipping upload.",
  );
  process.exit(0);
}
...
if (!existsSync(distDir)) {
  console.log(
    `[sentry-sourcemaps] ${distDir} doesn't exist — skipping (did the build actually run?).`,
  );
  process.exit(0);
}
